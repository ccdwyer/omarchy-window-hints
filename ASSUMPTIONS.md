...
- **No in-process service lookup.** Overlay does not call `pluginRegistry.serviceFor`, `shell.serviceFor`, or `shell.firstPartyServiceFor` (those are not in the Quattro IPC contract). Overlay is display-only: it paints Session and forwards every key / begin / end to the extra IPC target (`omarchy-shell window-hints key|begin|end`). It does not run Input, dispatch, or the watchdog.
- **IPC.** Summon/hide/toggle use `omarchy-shell shell summon|hide|toggle <id>` (Quattro shell target) via `Quickshell.execDetached` — not `shell.summon` / `shell.hide` methods. **Hint keys use a distinct registered IPC target** `window-hints` (`omarchy-shell window-hints key a`), matching extra plugin targets such as `image-selector`. That IpcHandler lives on the keepLoaded Service. Overlay.`key()` forwards to that target; it never re-issues `shell call <id>` (that was a recursion). `status` remains `shell call <id> status '{}'`.
- **Kinds include `bar-widget`.** Marketplace listing requires a bar chip. `BarWidget.qml` is registered in `entryPoints.barWidget` with a `barWidget` metadata block (`displayName`, `category`, `defaultSection: right`, `defaults`, `schema`). Clicking the chip summons the overlay when binds are live; if they are not, the chip shows **Set hotkey** / **Install hints submap** instead of auto-assigning.
...
- **`suggestedBind`** is parsed (`SUPER+H` → mods `SUPER`, key `H`) and passed to `binds-check`. Collision detection uses that pair. Generated Lua / keyword batches (`hints-ctl submap install|script <bind>`) emit that bind, not a hardcoded Super+F. Default summon is **Super+H**. Super+F is Omarchy fullscreen and is never written or unbound.
- **Keybinds are opt-in from the bar.** First load never writes `bindings.lua` and never auto-installs the `hints` submap. Only an explicit **Set hotkey** / **Install hints submap** click (IPC `installBinds opt-in`) calls `compat/install-binds.py --summon`, which writes the marked `hl.bind` + `hl.define_submap("hints")` block directly to `~/.config/hypr/bindings.lua`. `installBinds auto` is refused. Summon candidates are Super+H, Super+semicolon, Super+Alt+F. Occupied combos are skipped; if all three are taken nothing is written. Never `hl.unbind`. Never stage generated Lua under `/tmp`; if `XDG_RUNTIME_DIR` is unset/unusable the `--file` path fails closed. Notify with `omarchy notification send` only after a successful write.
...
## Hyprland (Omarchy Quattro / 0.56 Lua)
...
- **Swap.** Probe is `hyprctl dispatch 'hl.dsp.window.swap({ target = "address:0x0" })'` — one Lua expression argv, matching Hyprland 0.56’s `dispatch <lua>` contract. Classic `swapwindow address:0x0` is a Lua syntax error (exit 7) and must not be used. Capable only if that call shows an address target was accepted (`Invalid window` / `Window not found` / `target window not found`). Directional-only usage keeps the verb greyed. Version is not used as a capability signal. Cross-workspace swap is never implemented.
...
- **Cold-start snapshot.** `clientsReady` / `monitorsReady` flip only when `hyprctl` exits 0 **and** the payload is a JSON array (`[]` is valid; `{}` / garbage is not). If the first summon arrives before that, the overlay still opens and grabs input, but labels are **not** frozen empty — `frozenOnUnreadyModel` rebuilds the invocation when both snapshots land. Live polling after a real freeze still only drops vanished labels.
...
- Cargo `target/` (including `src/hints-ctl/target/`) is local build output. It is gitignored and `export-ignore`d so clones and git-archive packages do not ship it. `cargo test` / `build.sh` may leave it on disk; that is not part of the plugin. The shipped helper is `bin/hints-ctl` (also gitignored) or `compat/hints-ctl.sh`.
