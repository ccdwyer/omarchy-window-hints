...
- **`suggestedBind`** is parsed (`SUPER+F` → mods `SUPER`, key `F`) and passed to `binds-check`. Collision detection uses that pair. Generated Lua / keyword batches (`hints-ctl submap install|script <bind>`) emit that bind, not a hardcoded Super+F. The shipped `bindings.lua` paste and the auto-installer default to **Super+H**. Super+F is Omarchy fullscreen and is never written or unbound.
- **Keybinds auto-assign on first load** appends a marked `hl.bind` + `hl.define_submap("hints")` block to `~/.config/hypr/bindings.lua` after `hyprctl -j binds`. If any of this plugin's binds is already live, skip (no notify spam). Summon candidates are Super+H, Super+semicolon, Super+Alt+F. Occupied combos are skipped; if all three are taken nothing is written. Never `hl.unbind`. Notify with `omarchy notification send` only after a successful write.
...
- **Cold-start snapshot.** `clientsReady` / `monitorsReady` flip only when `hyprctl` exits 0 **and** the payload is a JSON array (`[]` is valid; `{}` / garbage is not). If the first Super+F arrives before that, the overlay still opens and grabs input, but labels are **not** frozen empty — `frozenOnUnreadyModel` rebuilds the invocation when both snapshots land. Live polling after a real freeze still only drops vanished labels.
