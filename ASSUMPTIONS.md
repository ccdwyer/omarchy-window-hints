...
- **No in-process service lookup.** Overlay does not call `pluginRegistry.serviceFor`, `shell.serviceFor`, or `shell.firstPartyServiceFor` (those are not in the Quattro IPC contract). Overlay keys run Input locally via the shared Session library. Overlay `open`/`close` kick the keepLoaded Service through the extra IPC target (`omarchy-shell window-hints begin|end`).
...
- **Cold-start snapshot.** `clientsReady` / `monitorsReady` flip only after a successful `hyprctl -j` parse (empty arrays count). If the first Super+F arrives before that, the overlay still opens and grabs input, but labels are **not** frozen empty — `frozenOnUnreadyModel` rebuilds the invocation when both snapshots land. Live polling after a real freeze still only drops vanished labels.
