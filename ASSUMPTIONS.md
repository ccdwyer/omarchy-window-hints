...
- **IPC verb** is `omarchy-shell shell call <id> <method> <arg>` and `shell summon|hide|toggle <id> <payloadJson>`. Confirmed in `quattro-shell-reference.md`. `call` invokes a method on the **host-loaded entry-point Item**. Service.qml and Overlay.qml both expose `key`, `summon`, `toggle`, `hide`, `dismiss`, `end`, `ping`, `status`, `begin` on the root. An `IpcHandler` of the same id remains as an extra Quickshell target, not the primary path. `status` is called as `call <id> status '{}'`.
